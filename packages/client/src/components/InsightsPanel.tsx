...
interface Props {
	...
	// v3 闭环①：学情→错题本下钻入口（bankId 级）
	onDrillWrong?: () => void;
	// 契约⑥-①：薄弱点 → 错题本带 tag 筛选（auditor 否决项，2026-08-12 补齐）
	onDrillWrongTag?: (tag: string) => void;
}
...
export function InsightsPanel({
	...
	return (
		<div className="card">
			<div
				className="row"
				style={{ gap: 28, alignItems: "center", marginBottom: 8 }}
			>
				<Ring pct={pct} />
				<div>
			...
			{weak.length === 0 ? (
				...
				weak.map((t) => {
					...
					return (
						<div key={t.tag} className="bar-row" style={{ gap: 8 }}>
							<div
								className={`bar-label${onDrillTag ? " clickable" : ""}`}
