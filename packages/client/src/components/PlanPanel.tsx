...
function PlanCard({
	...
	const totalCount =
		tasks?.length ?? plan.phases.reduce((s, p) => s + p.tasks.length, 0);
		...
		>
			<span
				style={{
					cursor: "pointer",
					fontSize: 16,
					color: task.status === "done" ? "var(--ok)" : "var(--muted)",
				}}
				onClick={() => {
			...
			<div style={{ flex: 1 }}>
				...
				<div className="muted" style={{ fontSize: 11 }}>
					...
					{task.scope.mode === "byTag" &&
						task.scope.tag &&
						` · ${task.scope.tag}`}
					{task.scope.mode === "byType" &&
						task.scope.type &&
						` · ${task.scope.type}`}
				</div>
			</div>
