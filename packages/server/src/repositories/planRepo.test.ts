...
const phases = [
	{
		...
		tasks: [
			{
				title: "重练代数",
				scope: { bankId: "b1", mode: "byTag" as const, tag: "代数" },
			},
	...
	},
	{
		...
		description: "综合",
		tasks: [
			{ title: "全量练习", scope: { bankId: "b1", mode: "all" as const } },
		],
	},
];
...
beforeAll(async () => {
	...
	dbMod
		.getDb()
		.prepare(
			`INSERT INTO banks (id, name, created_at) VALUES ('b1', '测试库', 0)`,
		)
		.run();
...
describe("planRepo（计划/任务两表，D2）", () => {
	...
	it("updateTaskStatus 置 done 记录完成时间，返回是否命中", () => {
		const plan = planRepo.createPlan("b1", "任务流", undefined, [
			{
				title: "p1",
				tasks: [{ title: "t1", scope: { bankId: "b1", mode: "all" } }],
			},
		]);
	...
	it("getPlan/listPlans 查询与状态更新", () => {
		...
		expect(all.some((p) => p.id === plan.id && p.status === "completed")).toBe(
			true,
		);
	});
