...
let buildTrendHistory: (bankId?: string) => import('@exam/shared').TrendHistoryResponse;
...
beforeAll(async () => {
  ...
  db = dbMod.getDb();
  db.prepare(`INSERT INTO banks (id, name, created_at) VALUES ('b1', '库1', 0)`).run();
...
describe('buildTrendHistory（双维时间序列，D34）', () => {
  ...
  it('整体曲线按时间正序（旧→新）', async () => {
    await saveSnapshot(randomUUID(), 'b1', 10, 5, [{ tag: '代数', total: 5, correct: 2 }]);
    await new Promise((r) => setTimeout(r, 2));
    await saveSnapshot(randomUUID(), 'b1', 10, 8, [{ tag: '代数', total: 5, correct: 4 }]);
  ...
  it('按库隔离：b2 快照不进 b1 趋势', async () => {
    await saveSnapshot(randomUUID(), 'b2', 10, 3, [{ tag: '英语', total: 10, correct: 3 }]);
