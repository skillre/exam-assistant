...
describe("validateDiagnosisResults（错因枚举固定）", () => {
	...
	it("非法错因枚举被拒绝", () => {
		expect(() =>
			validateDiagnosisResults([
				{ ...valid[0]!, errorCategory: "random_guess" },
			]),
...
describe("validatePlan（阶段+任务含 PracticeScope）", () => {
		...
		phases: [
			{
				...
				tasks: [
					{
						title: "重练代数 10 题",
						scope: { bankId: "b1", mode: "byTag", tag: "代数" },
					},
				],
			},
		],
		...
		expect(() =>
			validatePlan({
				...
				phases: [
					{
						...validPlan.phases[0]!,
						tasks: [{ title: "x", scope: { bankId: "b1", mode: "byRandom" } }],
					},
				],
			}),
		).toThrow(/mode|enum/);
	...
	it("空阶段被拒绝", () => {
		expect(() => validatePlan({ title: "空计划", phases: [] })).toThrow(
			/阶段|至少/,
		);
	});
