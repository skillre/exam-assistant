...
describe('parseFileBuffer JSON', () => {
  it('合法数组解析为草稿', () => {
    const drafts = parseFileBuffer('t.json', Buffer.from(JSON.stringify([single])));
  ...
  it('畸形 JSON 抛可读错误', () => {
    expect(() => parseFileBuffer('t.json', Buffer.from('{bad json'))).toThrow('JSON 文件无法解析');
  });

  it('结构不符（缺字段/错类型）被 zod 拒绝', () => {
    expect(() => parseFileBuffer('t.json', Buffer.from(JSON.stringify([{ stem: '缺字段' }])))).toThrow();
    expect(() =>
      parseFileBuffer('t.json', Buffer.from(JSON.stringify([{ ...single, answer: '不是数字' }]))),
    ).toThrow();
  });

  it('非法题型枚举被拒绝', () => {
    expect(() =>
      parseFileBuffer('t.json', Buffer.from(JSON.stringify([{ ...single, type: 'essay' }]))),
    ).toThrow();
  });
});
...
describe('parseFileBuffer CSV', () => {
  ...
  it('畸形 CSV（空文件/只有表头）不抛非预期异常', () => {
    ...
    expect(() => parseFileBuffer('t.csv', Buffer.from('type,stem,options,answer\n'))).not.toThrow();
  });
});
...
describe('parseFileBuffer xlsx', () => {
  ...
  it('非 xlsx 内容（伪文件）容错不崩', () => {
    expect(() => parseFileBuffer('t.xlsx', Buffer.from('not an xlsx at all'))).not.toThrow();
  });
});
