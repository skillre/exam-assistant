...
const aiStub = {
  ...
  testProvider: async (): Promise<{ ok: boolean; message: string }> => ({ ok: true, message: 'ok' }),
  listModels: () => [],
  setModel: () => {},
  runOnce: async () => '[]',
...
describe('HTTP 全链路（routes.smoke，评审 S3）', () => {
  it('建库 → 导入（commit 2 题）', async () => {
    ...
    const imp = await post('/api/import/commit', {
      bankId,
      questions: [
        { type: 'single', stem: '1+1=?', options: ['1', '2'], answer: 1, explanation: '', tags: [] },
        { type: 'boolean', stem: '地球是圆的', options: [], answer: true, explanation: '', tags: [] },
      ],
    });
  ...
  it('练习：start → 判分 2 题 → graded 返回完整判分态', async () => {
    const s = await post('/api/practice/start', { scope: { bankId, mode: 'all' } });
    ...
    const g2 = await post('/api/quiz/grade', { questionId: q2, userAnswer: false });
    expect(g1.json().isCorrect).toBe(true);
    expect(g2.json().isCorrect).toBe(false);

    const graded = await app.inject({ method: 'GET', url: `/api/practice/${session.id}/graded` });
    const g = graded.json() as Record<string, { attemptId: string; isCorrect: boolean; correctAnswer: unknown; answeredAt: number }>;
  ...
  it('错题本：答错的题出现在列表', async () => {
    const w = await app.inject({ method: 'GET', url: `/api/quiz/wrong?bankId=${bankId}` });
    const items = w.json() as { question: { id: string }; wrongCount: number }[];
  ...
  it('provider：配 Key 入库 → 列表脱敏（无 apiKey 字段）', async () => {
    ...
    const providers = list.json() as { name: string; configured: boolean; apiKey?: string }[];
  ...
  it('删库级联：题目/错题清空', async () => {
    const del = await app.inject({ method: 'DELETE', url: `/api/banks/${bankId}` });
    expect(del.statusCode).toBe(204);
    const qs = await app.inject({ method: 'GET', url: `/api/banks/${bankId}/questions` });
