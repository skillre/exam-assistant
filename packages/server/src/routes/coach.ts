...
export function registerCoachRoutes(app: FastifyInstance, ai: AiService): void {
	...
	app.patch<{
		Params: { id: string };
		Body: { status: LearningTask["status"] };
	}>("/api/coach/task/:id/status", async (req, reply) => {
		const { status } = req.body ?? {};
		if (!status || !["pending", "in_progress", "done"].includes(status)) {
			return reply
				.code(400)
				.send({ error: "status 必须是 pending/in_progress/done" });
		}
