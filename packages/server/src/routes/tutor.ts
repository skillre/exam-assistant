...
export function registerTutorRoutes(app: FastifyInstance, ai: AiService): void {
    ...
    async (req, reply) => {
      ...
      try {
        ...
        if (existing) {
          ...
          const created = await ai.createTutorSession({ systemPrompt: TUTOR_SYSTEM_PROMPT });
          session = created.session;
          tutorSessionRepo.create(questionId, created.jsonlPath);
          firstPrompt = buildExplainPrompt(question, attempt.userAnswer, attempt.isCorrect);
        }
  ...
  app.post<{ Body: { questionId?: string; message?: string } }>(
    ...
    async (req, reply) => {
      ...
      if (!record) {
        return reply.code(409).send({ error: '该题尚无答疑会话，请先请求讲解' });
      }
  ...
  app.get<{ Querystring: { questionId?: string } }>('/api/tutor/history', async (req, reply) => {
