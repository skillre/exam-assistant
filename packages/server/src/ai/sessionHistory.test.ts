...
function makeJsonl(lines: { role: 'user' | 'assistant'; text: string }[]): string {
  return lines
    .map((l, i) =>
      JSON.stringify({
        type: 'message',
        ...
        message: { role: l.role, content: [{ type: 'text', text: l.text }], timestamp: 0 },
      }),
    )
    .join('\n');
}

function writeSession(lines: { role: 'user' | 'assistant'; text: string }[]): string {
...
describe('readSessionHistory', () => {
  it('默认 skipFirstUserTurn=false：返回全部 user/assistant 轮（collector 行为不变）', () => {
    ...
    expect(turns[0]).toEqual({ role: 'user', content: '首条注入的讲解上下文：题目+正确答案（应被跳过）' });
  ...
  it('skipFirstUserTurn=true：跳过首条 user 轮，其余顺序不变', () => {
    ...
    expect(turns[0]).toEqual({ role: 'assistant', content: '这道题的答案是 B，因为…' });
  ...
  it('非 message 条目与空文本被忽略', () => {
    ...
    writeFileSync(
      path,
      [
        JSON.stringify({ type: 'thinking', id: 't1', parentId: null, timestamp: '', thinking: '…' }),
        ...makeJsonl([{ role: 'user', text: '   ' }, { role: 'user', text: '有效追问' }]).split('\n'),
      ].join('\n'),
    );
