...
function TaskCard({ task, onStart, onStatusChange }: {
  ...
  return (
    ...
    >
      <span
        style={{ cursor: 'pointer', fontSize: 16, color: task.status === 'completed' ? 'var(--ok)' : 'var(--muted)' }}
        onClick={() => {
      ...
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 13, textDecoration: task.status === 'completed' ? 'line-through' : 'none' }}>
          {task.title}
        </div>
